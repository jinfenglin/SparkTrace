...
public class CFNodeBuilder {
    ...
    public CFNodeBuilder(TraceComposite tc, String cfNodeId) {
        this.tc = tc;
        this.cfNodeId = cfNodeId;
    }

    public CFNode buildCFNode() throws Exception {
        CFNode cfNode = new CFNode(this.cfNodeId);
        for (IOItemDefinition inputField : tc.getInputs()) {
            cfNode.addInputField(inputField.getFieldName());
        }
        for (IOItemDefinition outputField : tc.getOutputs()) {
            cfNode.addOutputField(outputField.getFieldName());
        }

        cfNode.setVertexLabel(tc.getName());
